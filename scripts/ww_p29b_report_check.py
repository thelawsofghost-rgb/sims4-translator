...
wrote and classifies.  Precedence (strict):
  ...
  3. a captured get_display_name root cause, strongest first:
        ...
        GET_DISPLAY_NAME_IS_SWITCH  (base TEST299 but get_display_name returns old)
  ...
  4. picker-stage branch (no in-gdn old-English switch proved):
        if a get_picker_row frame shows PICKER_ROW_TEXT == 'Caught Cheating 1'
            while a gdn returning TEST299 exists -> PICKER_ROW_USES_OTHER_SOURCE
        else (all observed gdn return TEST299, rows have no old English)
...
def main():
        ...
        for ln in f:
            ...
            if s.startswith("HOOK_INSTALLED=YES"):
                ...
                if v in _ORDERED_GDN:
                    gdn_verdicts[v] += 1
            elif s.startswith("BASE_DISPLAY_NAME='TEST299'"):
                gdn_base_test299 = True
                any_target = True
            elif s.startswith("GET_DISPLAY_NAME_RETURN='TEST299'"):
                gdn_returned_test299 = True
                any_target = True
            elif s.startswith("PICKER_#"):
                picker_frames += 1
                any_target = True
            elif s.startswith("PICKER_ROW_TEXT='Caught Cheating 1'"):
                picker_row_old = True
                any_target = True
    # 1. error precedence
    if hook_error:
    ...
    if any_target and picker_frames:
        if picker_row_old and gdn_base_test299:
            # branch C: gdn returned TEST299 but picker row shows old English
