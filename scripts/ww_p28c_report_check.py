...
对 output/ww_p28c/ww_p28c_report.txt + 实际生成包做独立二次验证:
  ...
  2. 独立重读 key (不信任 report 文本值本身, 从纯 ASCII key-path 提取):
       ...
       TGI_MATCH=YES / INSTANCE=0x43F3438A94EDEB2B / TARGET_ORDINAL=299
       TARGET_NEW_RAW=TEST299 / TARGETS_CHANGED=1/1 / NON_TARGET_XML_DIFF=0
  3. 独立对【真实生成包 bytes】逐字节复核 (不读 report 的 sha):
       ...
       c. 解压新 XML 中唯一出现 <T n="animation_raw_display_name">TEST299</T> 一次
       d. ordinal 299 语义命中 (自元数据多源交叉, ASCII)
...
def main():
    ...
    expect_keys = [
        ...
        "INSTANCE", "TARGET_ORDINAL", "TARGET_NEW_RAW", "TARGETS_CHANGED",
    ...
    # 独立解析 XML: 唯一 TEST299 出现; ordinal 299 命中
    txt = decompress_maybe(body).decode("utf-8", errors="replace")
    if txt.count(f">{TARGET_NEW_RAW}<") != 1:
        print(f"TEST299_OCCURRENCE={txt.count(f'>{TARGET_NEW_RAW}<')} != 1")
        print("REPORT_CHECK=FAIL")
        return 4
    xml_ok = True
    try:
