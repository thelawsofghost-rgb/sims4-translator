...
# -*- coding: utf-8 -*-
"""
ww_p28c_cfg_audit.py —— P28C 专属 Resource.cfg 只读审计 + 决策 (与 P27/P28A/P28B 隔离)

fork 自 ww_p28b0_cfg_audit.py 的已实证逻辑 (glob/eff/Sims4 高数值优先/fail-closed),
但【硬编码 P28C 专用语义与输出键】, 绝不与 P27_Overrides / P28B_Overrides / P28A 混用:
...
两模式:
  ...
  propose : check + 输出待追加 ASCII 行(base64) 与建议 Priority(全局最高+margin=100),
            供 ww_p28c_deploy.ps1 精确追加.
Sims4 precedence: 数值越大 priority 越高, 高者覆盖低者.
目标必须可证明: P28C_OVERRIDE_EFFECTIVE_PRIORITY > SOURCE_EFFECTIVE_PRIORITY。

输出键 (ASCII):
  RESOURCE_CFG_EXISTS / RESOURCE_CFG_SHA_BEFORE / PRIORITY_MIN/MAX/COUNT / RULE_COUNT
  MODS_ROOT / SOURCE_FULL_PATH / SOURCE_REL_PATH / OVERRIDE_TARGET_REL_PATH
  SOURCE_EFFECTIVE_PRIORITY
  P28C_OVERRIDE_EFFECTIVE_PRIORITY
  P28C_DIR_DEDICATED_RULE=YES|NO / P28C_DEDICATED_PRIORITY=<int>
  PRIORITY_RELATION=OVERRIDE_HIGHER / PROPOSED_PRIORITY / APPEND_REQUIRED / APPEND_LINES=<base64>
...
用法:
  python scripts\\ww_p28c_cfg_audit.py check  "<Resource.cfg>"
  python scripts\\ww_p28c_cfg_audit.py propose "<Resource.cfg>"
(只写后 post-write re-audit 同样调用本脚本 check/propose 于【写入后】的 cfg 验证实际结果.)
"""
import base64
import hashlib
import re
...
def main():
    if len(sys.argv) < 3:
        print("usage: ww_p28c_cfg_audit.py check|propose <resource.cfg>")
        return 2
    ...
    override_rel = f"{OBJECT_DIR}/{OVERRIDE_BASENAME}"
    ...
    print(f"OVERRIDE_TARGET_REL_PATH={override_rel} (virtual)")

    s_cov, s_prio = classify(rules, source_rel)
    o_cov, o_prio = classify(rules, override_rel)
    print(f"SOURCE_PKG_COVERED={'YES' if s_cov else 'NO'}")
    print(f"SOURCE_PKG_PRIO={s_prio if s_cov else 0}")
    print(f"OVERRIDE_COVERED={'YES' if o_cov else 'NO'} (generic glob)")
    print(f"OVERRIDE_PKG_PRIO={o_prio if o_cov else 0}")
    ...
    print(f"P28C_DIR_DEDICATED_RULE={'YES' if dedicated else 'NO'}")
    print(f"P28C_DEDICATED_PRIORITY={ded_prio if ded_prio is not None else 0}")
    ...
    ovr_eff = eff(override_rel)

    if src_eff is None:
        print("SOURCE_EFFECTIVE_PRIORITY=UNRESOLVED")
        print("P28C_OVERRIDE_EFFECTIVE_PRIORITY=UNRESOLVED")
        print("VERDICT=FAIL")
        print("REASON=SOURCE_PRIORITY_UNRESOLVED")
        return 4

    print(f"SOURCE_EFFECTIVE_PRIORITY={src_eff}")
    print(f"P28C_OVERRIDE_EFFECTIVE_PRIORITY={ovr_eff if ovr_eff is not None else 'UNRESOLVED'}")

    s_hits = [(r["prio"], r["pattern"]) for r in rules if glob_match(r["pattern"], source_rel)]
    o_hits = [(r["prio"], r["pattern"]) for r in rules if glob_match(r["pattern"], override_rel)]
    ...
    print(f"OVERRIDE_MATCH_COUNT={len(o_hits)}")
    for i, (pr, pat) in enumerate(sorted(o_hits, reverse=True), 1):
        print(f"OVERRIDE_MATCH_RULE_{i}_PRIORITY={pr}")
        print(f"OVERRIDE_MATCH_RULE_{i}_PATTERN={pat}")
