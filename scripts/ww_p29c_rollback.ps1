# ww_p29c_rollback.ps1 -- P29-C rollback (Windows real machine).
# Removes ONLY P29-C effects:
#   1. delete Mods\ww_p29c_display_caller_trace.ts4script
#   2. delete the log (ww_p29c_display_caller_trace.log) under the writable roots used
#   3. if this P29-C run redeployed P28C (flag), restore the base via P28C rollback
#   4. delete the flag
...
$LOG_NAME      = "ww_p29c_display_caller_trace.log"
...
if (Test-Path -LiteralPath $FLAG_P28C_DEP) {
    ...
    if (Test-Path -LiteralPath $P28C_ROLLBACK) {
        & $P28C_ROLLBACK
        if ($LASTEXITCODE -ne 0) { Fail "P28C rollback failed after p29c" }
    } else {
